...
class RunTestKarma extends PreparePackageClient {
    constructor(name, taskDepend, taskDependencies, gulp, helper, conf, project) {
        ...
        if (fs.existsSync(path.join(project.dir, this.defaultTestsFileName))) {
            ...
            helper.debug("Le fichier de test pour karma 'tests.webpack.js' est introuvable");
        }
    ...
    task(gulp, helper, conf, project) {
        return (done) => {
            if (helper.isSkipTests()) {
                helper.info("Exécution des tests karma annulée car l'option '--skipTests' a été utilisée.");
                return done();
            }
            ...
            if (this.karmaConfigFile) {
                ...
                this.configuration.webpack = merge(
                    conf.webpack,
                    merge(this.webpackConf(helper, conf, project, this.configuration), this.configuration.webpack),
                );
            ...
            if (!helper.getFile() && !helper.fileExists(this.configuration.files[0])) {
                helper.info("Exécution des tests Karma annulée car aucun tests à exécuter.");
                return done();
            }
                ...
                () =>
                    new Server(this.configuration, (exitCode) => {
                        if (exitCode && helper.getStopOnError()) {
                            helper.error("Arrêt du builder demandé (StopOnError).");
                            process.exit(1);
    ...
    webpackConf(helper, conf, project, confKarma) {
        ...
        const sassConfiguration = webpackConfigPart.addScssLoader(project, conf, helper);
        ...
        if (helper.isWebpackVisualizer()) {
            ...
            testConfiguration.plugins.push(
                new BundleAnalyzerPlugin({
                    reportFilename: path.resolve(path.join(project.dir, "dev", "webpack-visualizer.html")),
                    analyzerMode: "static",
                    openAnalyzer: false,
                }),
            );
        ...
        // ajout des externals
        const confWithExternals = this.buildExternal(conf.karma, helper);
        ...
        configuration.resolve.alias["./appenders/console"] = path.resolve(
            path.join(project.dir, helper.NODE_MODULES, "log4js", "lib", "appenders", "console.js"),
        );
        configuration.node = {
            __filename: "mock",
            __dirname: "mock",
