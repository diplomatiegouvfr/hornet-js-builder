class RemapCoverageCollector {
    ...
    updateBranch(source, srcItem, hits) {
        ...
        let key = ["b"];
        srcItem.locations.map((loc) => key.push(loc.start.line, loc.start.column, loc.end.line, loc.end.line));
    ...
    updateFunction(source, srcItem, hits) {
        ...
        const key = ["f", srcItem.loc.start.line, srcItem.loc.start.column, srcItem.loc.end.line, srcItem.loc.end.column].join(":");
    ...
    updateStatement(source, srcItem, hits) {
        ...
        const key = ["s", srcItem.start.line, srcItem.start.column, srcItem.end.line, srcItem.end.column].join(":");
...
module.exports = RemapCoverageCollector;
